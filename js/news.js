...
function srcSec(year) {
  return `
    <section class="main-section" id="${year}">

    </section>`
}

  function srcHead(title, date) {
    return `
    <header>
        <span>
          <strong>${title}</strong>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let elem = document.querySelector("#news-info");
  for(let year in newsData){

    elem.insertAdjacentHTML( 'beforeend', srcSec(year));
  }
  ...
  setTimeout(()=>{
    ...
    elems.forEach(el   => {
      let year = String(el.id);
      for (const data in newsData[year]) {
        el.insertAdjacentHTML( 'beforeend', srcHead(newsData[year][data].title, newsData[year][data].date));
      }
    });
